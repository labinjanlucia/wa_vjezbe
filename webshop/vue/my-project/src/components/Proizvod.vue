...
onMounted(async () => {
try {
const response = await axios.get(`http://localhost:3002/proizvodi/${props.id}`);
...
try {
let response = await axios.post('http://localhost:3002/narudzbe', podaci.value); 
  ...
  <div class="mt-4">
    <ul role="list" class="list-disc space-y-2 pl-4 text-sm">
      <li v-for="(value, key) in proizvod.karakteristike" :key="key" class="text-gray-400">
        <span class="text-gray-600 capitalize"><strong>{{ key }}:</strong> {{ value }}</span>
      </li>
